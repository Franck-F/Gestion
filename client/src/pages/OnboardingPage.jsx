...
function StepWelcome({ user, onNext }) {
  return (
    <div className="text-center max-w-md mx-auto">
      ...
      <p className="text-surface-500 text-base sm:text-lg mb-2">
        MyCheckList va vous aider à organiser toutes vos démarches.
      </p>
...
function StepGoal({ selected, onSelect, onNext, onBack }) {
  return (
    <div className="max-w-lg mx-auto">
      <h2 className="text-xl sm:text-2xl font-bold font-heading text-surface-900 text-center mb-2">
        Quel est votre objectif ?
      </h2>
      <p className="text-surface-400 text-xs sm:text-sm text-center mb-6 sm:mb-8">
        Vous pourrez toujours utiliser tous les modules.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-6 sm:mb-8">
        {GOALS.map(g => (
          <button
            ...
            className={`flex items-center gap-3 p-3 sm:p-4 rounded-xl border-2 text-left transition-all cursor-pointer ${
              selected === g.id
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      {(goalType === 'alternance' || goalType === 'stage' || goalType === 'emploi') && (
        <div className="bg-white rounded-xl border border-surface-200 p-3 sm:p-5 mb-4 sm:mb-5">
          <div className="flex items-center gap-2 mb-3">
            <Briefcase size={16} className="text-primary-500" />
            <h3 className="font-semibold text-surface-800 text-xs sm:text-sm">Première candidature</h3>
            <span className="text-[10px] sm:text-xs text-surface-400 ml-auto">Optionnel</span>
          </div>
      ...
      <div className="bg-white rounded-xl border border-surface-200 p-3 sm:p-5 mb-6 sm:mb-8">
        <div className="flex items-center gap-2 mb-3">
